...
class MyCSA1(nn.Module):#sota是5层
    def __init__(self, text_size, audio_size, nhead, dropout):
        ...
        # self.layer3 = my_attention1(text_size, audio_size, nhead, dropout)
        # self.layer4 = my_attention1(text_size, audio_size, nhead, dropout)
        # self.layer5 = my_attention1(text_size, audio_size, nhead, dropout)
...
class AMOA(nn.Module):#sota
    def __init__(self, text_size, audio_size, video_size, nhead, dropout, q_size, contrastive):
        ...
        if self.contrastive:
            ...
            self.tav_q = torch.randn([q_size,1582]).cuda()
            self.IS_q = torch.randn([q_size,1582]).cuda()
...
class _model(BertPreTrainedModel):
    def __init__(self, config, num_labels, q_size, alpha, contrastive, dropout):
        super(_model, self).__init__(config)
        ...
        self.AMOA = AMOA(768, 1582, 711, 1, dropout, self.q_size, self.contrastive)
    ...
    def forward(self, input_ids, all_audio_data,token_type_ids=None, attention_mask=None, labels=None, IS=None, video=None):
        ...
        text_audio, loss_c  = self.AMOA(encoder_lastoutput, all_audio_data1, attention_mask=extended_attention_mask, IS=IS, video=all_video_data1)
        ...
        logits1=logits
        labels_tmp = labels
        BCEWithLogitsLoss_f = nn.MSELoss()
